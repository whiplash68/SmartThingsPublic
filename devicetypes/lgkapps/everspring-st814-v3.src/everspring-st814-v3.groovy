...
preferences {
    input("TempChangeAmount", "number", title: "Temperature Change Amount?", range: "0..70",description: "The degrees the temperature must changes before a report is sent?", defaultValue: 2,required: false)
    input("HumidChangeAmount", "number", title: "Humidity Change Amount?",range: "0..70" ,description: "The percent the humidity must changes before a report is sent?", defaultValue: 5,required: false)
...
def parse(String description)
{
	...
	if( get_battery == true ) 
	{
        ...
        // Update to the last poll if there is one
        if(!state.lastTime)
        {
        	log.debug "State last time is null"
        	state.lastTime = new Date().time
        }
        
        def age = (new Date().time - state.lastTime)/60000
        //if (last)
        //{
        	/* device wakes up roughly every hour */
		//	age = last ? (new Date().time - last.date.time)/60000 : 10
        //}	
...
// fix interval if bad
if (settings.ReportTime <0)
  {
    settings.ReportTime =0
    log.debug "Time Interval too low... resetting to 0"
    }
...
if (settings.TempChangeAmount < 0)
  {
    settings.TempChangeAmount = 0
    log.debug "Temperature Change Amount too low... resetting to 0"
    }
 ...
 if (settings.HumidChangeAmount < 0)
  {
    settings.HumidChangeAmount = 0
    log.debug "Humidity Change Amount too low... resetting to 0"
    }
